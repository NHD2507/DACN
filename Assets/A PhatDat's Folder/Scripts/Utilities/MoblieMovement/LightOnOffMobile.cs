...
public class LightOnOffMobile : MonoBehaviour
{
    public bool PlayerInZone; // Kiểm tra xem người chơi có trong vùng không
    ...
    public static LightOnOffMobile currentTarget;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "Reach") // Nếu người chơi trong vùng
        {
    ...
    private void OnTriggerExit(Collider other) // Nếu người chơi rời khỏi vùng
    {
        if (other.gameObject.tag == "Reach")
        {
            PlayerInZone = false;
    ...
    public void ToggleLight() // Phương thức bật/tắt đèn
    {
        if (PlayerInZone == true) // Chỉ bật/tắt đèn khi người chơi trong vùng
        {
            IsPower = !IsPower;
            switchON.SetActive(!switchON.activeSelf);
            switchOFF.SetActive(!switchOFF.activeSelf);
            ...
            audioSource.Play();
            Debug.Log("Bật/tắt đèn. Trạng thái hiện tại: " + IsPower);
        }
        else
        {
            Debug.Log("Player không ở trong vùng tương tác. Không thể bật/tắt đèn.");
        }
    }
